...
ソフトウェア設計について、再考する時期がやってきた。前回はフラー株式会社でクリーンアーキテクチャ導入時（2022年頃）、2024年終わりにDDDやマイクロアーキテクチャを真剣に学習した時期であろう。今回は、カルチャーショックを受けたことがキッカケで、再考する。現職のカンムではクリーンアーキテクチャを採用しておらず、かつ関数分割もそこまでされていない状況であり、今までとは180度文化圏が異なる。
