...
夢の中で、いつも利用している在来線に乗り、見知らぬ場所についた。乗客全員が予想外の出来事で驚いていた。寂れた感じがするが、開放感のある出口だった。そこで飲み会をして、「帰りの時間に間に合うかな」と思いながら駅に戻ったら、両親が座っていて私を待っていた。驚きで目が冷めてしまった。私が見る夢のレパートリーの一つに、寂れた場所を歩き、元いた場所に戻ってくるというものがある。このパターンは、夢を見ていることに気づけていない。

朝、息子が「くろいはちみつがおいしいよ」と言ってたので、食パンに塗って食べさせた。黒いハチミツは蕎麦の花の蜜らしい。ロシアの家族から国際便で届いた。いつもナッツとハチミツが届くので「これでハニーナッツが作れるのでは?」と思っている。息子にパンを渡す時、ハチミツをスプーンで塗る仕草をわざとらしく見せつけたら、案の定パンの上のハチミツをスプーンで伸ばしていた。今は人の真似をする時期らしい。

父親と息子と一緒に[みどりと森の運動公園](https://www.city.niigata.lg.jp/kurashi/park/shoukai/area/nishiku/p_midorimori.html)へ行き、息子がキックボードをするのを見守った。息子はキックボードを蹴りながら、道中で息絶えそうなミミズをチェックしていた。元気なミミズが居たが、大半は天に召されていた。ミミズは雨が降ると移動するために地上に出てくるが、雨が降っていない日だったので土中が暑かったのかもしれない。息子は、カエルやバッタを見つけると、棒で突っついていた。同年代ぐらいの子供と一緒に遊びたそうな素振りをすべり台近くでしていたが、息子が奇声を上げているので相手の子が離れていっていた。

今日は、これからFlutterのコードを読んでから、散髪に行って終わる。Flutterのコードは、Claudeに生成させて、自分はそれを読むだけだ。勉強中に「[かろやか浅煎りアメリカーノ](https://products.suntory.co.jp/d/4901777435612/)」を飲んだが、味が薄すぎた。飲めるが、コーヒーらしさがなかった。

......残念ながら、Flutterの開発環境をParrotOSに作ろうとすると、依存関係地獄に陥ることが分かった。まずは開発環境をUbuntuに戻す。私は10年ほどLinuxユーザーだが、マイナーなディストリを使うと開発環境を整えるのに時間がかかることに不満がある。Linuxをガチャガチャ弄ることに楽しさを感じない年頃なので、ある程度の安定性が欲しい。可処分時間が少ないので、環境構築で開発時間が減るのが勿体ない。パッケージの新しさ、環境構築の楽さはUbuntuが最善だと思う。他のUbuntu派生ディストリは、様々な言語の開発環境を構築する時に破綻しやすい。

## 2025年8月15日

損をしたくない。私を知る者は「お前は金に意地汚いからな」と考えるかもしれないが、技術習得の話だ。ソフトエンジニアは、落ち目の技術や自分が利用しない競合技術を積極的に学びたくない場合がある。例えば、COBOLを今更学びたいと考えるだろうか。iOSエンジニアがFlutterやReact Nativeを極めようと思うだろうか。私は今、「LLMをGUIアプリ習得で使おう」と考えており、フレームワークの選択肢がいくつかある。その中で損をしないのはどれだろうかと考えている。ちなみに、N度目の学習になる。クロスプラットフォームで利用できるフレームワークは、利用者が「全部、辛いところがある」と話している印象がある。その一方でネイティブ（つまり、SwiftやKotlin）を選ぶと、デザインと真剣に向き合う必要がある。各社の利用状況を見ると、Flutterの人気が高そうに見えており、自分が学ぶならFlutter（Dart）が良いかなと結論付けている。実用性を考えるとWebフロントを学ぶのがいいのだが、Webフロントは好きじゃない。

正直なところ、損はしたくないが、何度もいいから一つ習得したい。何も習得できないことが最も性質が悪い。私はFlutter、Android（Kotlin）、Next.js、Kotlin Multiplatformを試した経験があるが、全部物にならなかった。今回のFlutterは、再挑戦する形になる。

ソフトウェア設計について、再考する時期がやってきた。前回はクリーンアーキテクチャ導入時（2022年頃）、2024年終わりにDDDやマイクロアーキテクチャを真剣に学習した時期であろう。今回は、カルチャーショックを受けたことがキッカケで、再考する。現職ではクリーンアーキテクチャを採用しておらず、かつ関数分割もそこまでされていない状況であり、今までとは180度文化圏が異なる。

検討に用いた参考文献は「[A Philosophy of Software Design](https://www.amazon.co.jp/-/en/Philosophy-Software-Design-John-Ousterhout/dp/1732102201)」であり、読了済みだが再度チェックした。私の現在の悩みに刺さった記述は、Deep module、Shallow Moduleだろう。Deep Moduleはインターフェースが狭く、実装が深い。Shallow Moduleはインタフェースが多く、実装が浅い。さらに刺さったのが[t-wada氏による補足](https://engineers.ntt.com/entry/2022/05/23/083118)で、「Shallow Moduleになりがちな子クラス主義」「子クラス主義の代表はJava、大クラス主義の代表はRuby」という補足は納得感があった。私はJavaの影響下にあったので、どちらかといえば実装はShallow Module寄りだろう。関数が20行を超えたら分割しろ、と言われたような世代だ。Deep Moduleとは縁がない。Deep Moduleに対する理解を深めるには、Ruby API設計者が書いた書籍（[「APIデザインケーススタディ―⁠―Rubyの実例から学ぶ。問題に即したデザインと普遍の考え方」](https://gihyo.jp/book/2016/978-4-7741-7802-8)）を読むのが良さそうだ。

今回の件のように、複数の文化圏に属することで多様な考え方に触れられる。一つの文化圏に長くいると、そこでの考え方を絶対視してしまう。コミュニティに広く参加するのは重要なのだ。私はコミュ障なので、コミュニティ探しが苦手なのだが、頑張らなければならない。

## 2025年8月14日

嫁の母親が体調悪く、ロシアに帰るかどうかの検討をしていた。ロシア帰省はいつでも急にやってくる。嫁が一人で帰る場合、格安航空券が10万円で、滞在費や経費でプラス10万がかかる。合計30万程度を見込んだ方が妥当だろう。2024年に嫁がロシアに帰省した時、「お金がないから二度と帰省できないと考えておいて」と伝えたにも関わらず、その翌年に旅費が発生している。年間の予算計画が狂う。ただし、今年は転職しており、去年と状況が異なる。帰省させるだけの余裕がある。そこで、嫁に交換条件かのように「今後は、仕事を選ばないで年間100万円ぐらい自分で稼いでくれ」と伝えて、ある程度納得してもらえた。話しぶりを聞いていると仕事を選びそうな雰囲気があったが、働いて貰えれば問題ない。我が家は、趣味に使う金額が低いので、この点を解消したい。今を生きていない感じがする。老後のために生きている。

饅頭（マントウ）を作ったが、柔らかい生地ではなく、生っぽく固い塊が出来上がった。二口食べて、相当不味いものを作ってしまったと後悔した。

## 2025年8月13日

私はエンジニアとしては三流だが、おかん属性で何とか生き残っている感じがする。ダースベイダーのテーマを口ずさみながら息子に近づき、「私がお前の父なのだ」と言い、爆笑をかっさらう遊びをした。なお、私も息子も、スターウォーズを見たことがない。世間の方々は、平気でダースベイダーの正体をネタバレしてくる。最も広まっているネタバレではないだろうか。

アーケード筐体の復刻版（10インチ程度のモニタ付きゲーム機）が昔から欲しい。ゲーセンに入り浸っていた日々が長いので、格ゲー筐体を自宅に置きたい気持ちが強い。自宅で懐かしい気持ちになりたいのだ。[ネオジオミニ](https://www.snk-corp.co.jp/neogeomini/)を持っているが、ゲームできる状態ではなかった。サイズが小さく、外部モニタとの相性も悪くて、殆どプレイできていない。SNKの格ゲーは餓狼MOWだけ好きなので、沢山ゲームが入っていても嬉しくなかった。ゲームパッドの操作性も悪く、そのパッドはPCでエミュを動かす時に使うものとしても不適切だった。

[「鍋に弾丸を受けながら」](https://comic-walker.com/detail/KC_003969_S?episodeType=first)で台湾料理として饅頭（マントウ）が書かれていて、食べたくなったから生地をこねた。饅頭とは、具の入っていないあんまんのようなものだ。中力粉が適していると書かれていたが、手に入らなかったので強力粉と薄力粉をブレンドした。明日、t-falの圧力鍋で蒸す。圧力鍋のサイズが小さいので、本格的な蒸し器が欲しい。

## 2025年8月12日

今日は夢で、ラーメン屋のバイトをさせられた。女性に接客したが、やたら腰低く謝罪させられて、提供したラーメンは1杯だけだったと思う。夢を見たということは、熟睡できたのだろう。

日経平均が終値42,718円と、過去最高だった。このタイミングで細かい株を処分できたのは良いが、買い増しタイミングを完全に見失っている。勿体なかったのはINPEXで、増配と自社株買いで2400円近くまで上がった。1900円台の時に買い増せば良かった。オルカンも基準価額28,972円。私のポートフォリオは、積み立て以外は暫く静観するしかない。

[虎金妃笑虎（ホロライブ）のコスプレをするユリコタイガー](https://www.instagram.com/reel/DFnR1vnvWyC/)のビジュアルが好みだが、私は虎金妃笑虎を知らないという不敬を犯している。今度動画を見ようと思う。予想だが、人気がなさそうな気がしている。YouTubeのおすすめに出てこないあたり、自分とはマッチしない可能性が高い。

昨日、連休が終わるのが嫌すぎて、スーパードンキーコング2をプレイしていた。フワフワと身体が浮く面まで行った。適当プレイでもクリアできる難易度なので、動画見ながら片手間で遊んでいた。もうゲームはプレイするより、LLMを使って作る方が楽しいかもしれない。しかし、画像を用意するのに強めのローカルマシンが必要だ。30万ぐらい散財したいが、もう少し円高を待ちたい。オッサンなので、一生待てるようになってきた。年単位で待ってしまう。

## 2025年8月11日

Claudeでゲームを作るのは不可能ではないと気づく。N度目の気づきだ。GitHub Copilotでは実力不足だったが、Claudeであれば作れそうな気がしている。今も、カードゲームを試しに作らせている。LLMや生成AIにゲームを作らせる試みは既に存在するようで、方向性としては「リアルタイムでイベントを生成する」があった。確かに、インタラクティブにストーリーや会話を生成する案は、現代版のローグライクゲームのようなものだ。正しい発想と言える。しかし、欠点としてはAPIキーが必要になることであろう。エンジニアしか遊べないゲームになってしまう。

昨日、息子と一緒に白雪姫（ディズニー）を見た。やはり途中で飽きてしまい、違うのに変えてほしいと頼んできた。ピノキオもそうだが、演出が非常に過剰で、例えば掃除のシーンが非常に長い。ストーリーに関係のない部分でアニメーションに力が入っている。PlayStation2時代のムービー偏重のゲームを思い出す。世界初の長編アニメ映画（1937年製）なので、現代のプロット重視の作風とは差があって当然だ。丁寧に描かれているので、白雪姫は芸術作品のように見える。しかし、子供が見るには退屈だろう。

## 2025年8月10日

嫁が語学サービス（italki）に登録するための文章を何度も添削した。添削対象は日本語だったが、ロシア語でも論理が破綻した文章を書いていそうな雰囲気だった。嫁はレビュー指摘を受けると不機嫌になったり、人の話に耳を貸さなくなる。伸び代はあるが、伸びる気配がないタイプだ。指摘を素直に受け入れないタイプは段々放置プレイされるので、普通の会社では上手くやっていけないだろうなと思う。そういう意味では、フリーランス的な稼ぎ方を目指しているのは正しい。

自室で息子の公文（宿題）を見守っていたが、息子の集中力が低い。真面目に宿題をやり続けるスキルが今のところない。小さい頃から訓練しないと、勉強できない子供が育つ理由が何となく分かる。いきなり小学1年生から長時間の授業を受けるのは、無理があると感じた。授業中に立ち上がる子供は、恐らく幼少時に家庭で教育を受けてこなかったのだろう（ADHDの可能性もあるが）。今から少しでも訓練するように、宿題に花丸を書いたり、天才だねと褒めたりしている。「てんさいじゃないよ」とニヤニヤした後に、息子は奇声を上げていた。
...
息子が歯を磨きながらベッドに転がり込んだ。自分と嫁が驚いた声を出したので、息子が泣き始めた。一息ついて、夕飯を食べた息子が飲み物を飲みながら椅子に座ろうとして、椅子が後ろに動いて転んだ。コップの水を顔に引っ掛けて泣いていた。泣いた後に、私がベリーを皿に移していたら、「自分がやりたかったのに」と泣き始めた。身体は大きくなったが、中身は子供だ。
